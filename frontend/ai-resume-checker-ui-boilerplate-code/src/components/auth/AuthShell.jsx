  ...
  return (
    <div className="hidden lg:block flex-1 relative rounded-[28px] overflow-hidden isolate">
      ...
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(circle, rgba(124,92,252,0.50) 0%, rgba(167,139,250,0.15) 40%, transparent 70%)",
          filter: "blur(60px)",
        }}
        animate={{
          x: [0, 40, 0],
...
export function AuthPrimaryButton({ children, disabled, ...props }) {
  return (
    <motion.button
      ...
      disabled={disabled}
      className="relative w-full h-12 rounded-2xl text-white font-semibold text-[15px] flex items-center justify-center gap-2 overflow-hidden shadow-[0_8px_24px_-8px_rgba(124,92,252,0.55)] transition-all duration-200 hover:shadow-[0_12px_28px_-8px_rgba(124,92,252,0.7)] hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed"
      style={{
        background:
...
export function AuthErrorBanner({ children }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className="text-xs text-[var(--danger)] bg-[rgba(239,68,68,0.08)] rounded-2xl px-4 py-2.5 leading-snug"
    >
      {children}
    </motion.div>
  );
