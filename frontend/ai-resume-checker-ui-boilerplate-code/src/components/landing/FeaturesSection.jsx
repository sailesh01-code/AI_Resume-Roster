...
export function FeaturesSection() {
  return (
    ...
    >
      {/* Atmospheric glow behind the section header */}
      <div
        aria-hidden
        className="absolute -top-20 left-1/2 -translate-x-1/2 w-[900px] h-[480px] pointer-events-none -z-10"
        style={{
          background:
            "radial-gradient(ellipse at center, rgba(124,92,252,0.14) 0%, rgba(124,92,252,0.04) 35%, transparent 70%)",
          filter: "blur(60px)",
        }}
      />
      ...
      <div className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
        {FEATURES.map((f, i) => (
          <motion.div
            ...
            className={`group relative rounded-[22px] border border-[var(--border)] shadow-card hover:shadow-hover hover:-translate-y-1 transition-all duration-300 overflow-hidden ${f.span || ""}`}
            style={{
          ...
          >
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(90deg, transparent, rgba(124,92,252,0.30), transparent)",
              }}
            />
            ...
            <div
              ...
              style={{
                background:
                  "radial-gradient(circle, rgba(124,92,252,0.22) 0%, rgba(124,92,252,0) 65%)",
                filter: "blur(20px)",
              }}
            />
            ...
            <div className="relative p-5 sm:p-6">
              <div className="flex items-start gap-3">
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, var(--accent-soft) 0%, rgba(124,92,252,0.15) 100%)",
                    boxShadow:
                      "inset 0 1px 0 0 rgba(255,255,255,0.7), 0 1px 2px rgba(124,92,252,0.08)",
                  }}
                >
...
export function SectionHeader({ eyebrow, title, sub, center = true }) {
  return (
    <div className={center ? "text-center max-w-3xl mx-auto" : "max-w-3xl"}>
      ...
      <h2
        className="font-display text-[34px] sm:text-[44px] lg:text-[52px] leading-[1.05] tracking-tight mt-4 animate-gradient"
        style={{
          backgroundImage:
            "linear-gradient(270deg, #7c5cfc, #06b6d4, #a855f7, #ec4899, #7c5cfc)",
  ...
  return (
    <div className="rounded-2xl bg-[var(--surface-2)] border border-[var(--border)] p-4">
      <div className="flex items-end justify-between mb-3">
        ...
        <div className="text-[10px] text-[#22c55e] bg-[rgba(34,197,94,0.1)] px-2 py-0.5 rounded-full font-semibold tabular">
          +18 pts
        </div>
      </div>
          ...
          <div key={b.label}>
            ...
            <div className="h-1.5 rounded-full bg-[var(--surface)] overflow-hidden">
              <motion.div
                ...
                style={{
                  background: "linear-gradient(90deg, #7c5cfc, #a78bfa, #06b6d4)",
                }}
              />
            </div>
          </div>
    ...
    <div className="rounded-2xl bg-[var(--surface-2)] border border-[var(--border)] p-4">
      ...
      <div className="flex flex-wrap gap-1.5 mb-2.5">
        {matched.map((k) => (
          <span
            key={k}
            className="px-2 py-0.5 rounded-full bg-[rgba(34,197,94,0.10)] text-[#22c55e] text-[10px] font-semibold"
          >
            ✓ {k}
          </span>
        ))}
      ...
      <div className="flex flex-wrap gap-1.5">
        {missing.map((k) => (
          <span
            key={k}
            className="px-2 py-0.5 rounded-full bg-[rgba(239,68,68,0.08)] text-[var(--danger)] text-[10px] font-semibold"
          >
            + {k}
          </span>
        ))}
...
function DiffPreview() {
  return (
    <div className="rounded-2xl bg-[var(--surface-2)] border border-[var(--border)] p-3 space-y-1 font-mono text-[11px]">
      <div className="flex gap-2 px-2 py-1 rounded-md bg-[rgba(239,68,68,0.06)]">
        <span className="text-[var(--danger)] font-bold w-3">−</span>
        <span className="text-[var(--ink-muted)] line-through">
          helped team
        </span>
      </div>
      <div className="flex gap-2 px-2 py-1 rounded-md bg-[rgba(34,197,94,0.06)]">
    ...
    <div className="rounded-2xl bg-[var(--surface-2)] border border-[var(--border)] p-4">
      ...
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-[90px]">
        ...
        <path
          d={path}
          fill="none"
          stroke="url(#featLineGrad)"
          strokeWidth="2.5"
          strokeLinecap="round"
        />
        <defs>
          <linearGradient id="featLineGrad" x1="0" y1="0" x2="1" y2="0">
